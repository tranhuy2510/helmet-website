...
exports.addToCart = (idUser, idProduct, quantity, size) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            db.query(checkSql, [validatedIdUser, validatedIdProduct, validatedSize], (err, result) => {
                if (err) return reject(err);

                if (result.length > 0) {
                    const updateSql = `UPDATE cart SET quantity = quantity + ? WHERE idUser = ? AND idProduct = ? AND size = ?`;
                    db.query(updateSql, [validatedQuantity, validatedIdUser, validatedIdProduct, validatedSize], (err, result) => {
                        if (err) return reject(err);
                        resolve({ message: 'Đã cập nhật giỏ hàng', affectedRows: result.affectedRows });
                    });
                } else {
                    const insertSql = `INSERT INTO cart (idUser, idProduct, quantity, size) VALUES (?, ?, ?, ?)`;
                    db.query(insertSql, [validatedIdUser, validatedIdProduct, validatedQuantity, validatedSize], (err, result) => {
                        if (err) return reject(err);
                        resolve({ message: 'Đã thêm sản phẩm vào giỏ hàng', insertId: result.insertId });
                    });
                }
            });
        } catch (err) {
        ...
        try {
            ...
            if (validatedQuantity <= 0) {
                ...
                db.query(deleteSql, [validatedIdCart], (err, result) => {
                    if (err) return reject(err);
                    resolve({ message: 'Item removed from cart', affectedRows: result.affectedRows });
                });
            } else {
                const updateSql = `UPDATE cart SET quantity = ? WHERE idCart = ?`;
                db.query(updateSql, [validatedQuantity, validatedIdCart], (err, result) => {
                    if (err) return reject(err);
                    resolve({ message: 'Cart quantity updated', affectedRows: result.affectedRows });
                });
            }
        } catch (err) {
            reject(err);
...
exports.addToCartGuest = (sessionCart, idProduct, quantity, size) => {
    try {
        ...
        if (validatedSessionCart[cartKey]) {
            ...
            validatedSessionCart[cartKey] = { idProduct: validatedIdProduct, quantity: validatedQuantity, size: validatedSize };
        }
...
exports.getGuestCartItems = (sessionCart) => {
    return new Promise((resolve, reject) => {
        try {
            const validatedSessionCart = validateSessionCart(sessionCart);
            if (!validatedSessionCart || Object.keys(validatedSessionCart).length === 0) return resolve([]);
            ...
            db.query(sql, productIds, (err, products) => {
                ...
                const cartItemsWithDetails = cartItems.map(cartItem => {
                    const product = products.find(p => p.idProduct === cartItem.idProduct);
                    return {
                        ...cartItem,
